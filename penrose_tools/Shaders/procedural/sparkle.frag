// sparkle.frag - Procedural Penrose with sparkling tile effect
#version 140
...
void main() {
    ...
    // Sparkle effect: random tiles flash brightly
    float sp = tile.tileId * 6.28318 + u_time * 2.0;
    float sk = pow(max(0.0, sin(sp)), 8.0);
    vec3 tileColor = mix(baseColor * 0.5, baseColor * 1.5, sk);
